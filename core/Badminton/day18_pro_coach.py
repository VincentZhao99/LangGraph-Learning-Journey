...
def generate_coach_report(action_type, metrics):
    ...
    # 💡 动态提示词：不同动作，不同的考核标准！
    if action_type == "高远球/杀球 (测上肢发力)":
        prompt = f"""你是一位极具耐心、专门指导 6-12 岁青少年的羽毛球亲子私教。
        学员（小朋友）刚才进行了【高远球】训练。CV底层提取的数据如下：
            - 引拍手肘夹角：{int(metrics['min_angle'])}度
            - 击球手肘夹角：{int(metrics['max_angle'])}度

        【青少儿评判标准注意】：
            小朋友力量不足，引拍往往会比较大或者比较小，击球时也不一定能完全伸直，这是正常的。

        请写一份【温柔、幽默、多鼓励】的诊断报告。
            1. 先大力表扬小朋友的努力（结合击球角度）。
            2. 用生动形象的比喻（比如变成小超人、拉弓射箭）指出引拍动作可以优化的点。
            3. 给家长提供 2 个能在家里和孩子一起玩的纠正小游戏。
        排版要活泼，多用 可爱/动物 emoji！"""
    else:
